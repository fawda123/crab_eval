...
selectInput(inputId = 'var3', label = 'Select third variable:', choices = c('Oxygen', 'Fluorescence', 'pH'), selected = 'Oxygen')
```
...
# three variables to model
kpvars <- reactive({
  
  # inputs
  var3 <- input$var3
  
  out <- c(var3, 'Temperature', 'Aragonite')
...
tomod <- reactive({
  ...
  out <- envdat %>%
    ...
    left_join(crbs, by = 'CTD') %>%
    gather('var', 'val', -CTD, -abundances, -pa, -Average.CL, -diss, -depth) %>%
    group_by(var) %>%
    mutate(
      val = scale(val)
    ) %>%
    spread(var, val) %>%
    dplyr::select_(.dots = c('depth', 'abundances', 'pa', 'Average.CL', 'diss', kpvars)) %>%
    gather('res', 'val', abundances, pa, Average.CL, diss) %>%
...
vifchk <- reactive({
  ...
  # get vif values
  out <- tocor %>% 
    dplyr::select_(.dots = c('depth', kpvars)) %>% 
    group_by(depth) %>% 
    nest %>% 
    mutate(
...
renderUI({
  ...
  selectInput("vr", "Choose variable:", choices = kpvars)
  
})
```

### Estimate guassian curve through x/y
...
renderUI({
  ...
  selectInput("dep", "Choose depth:", choices = unique(resopt$dep), selected = '100')
...
medests <- reactive({
  ...
  # median centroid estimates
  medests <- resopt %>%
    filter(vr == vrflt & dep %in% c(dep1, dep2)) %>%
    select(-mod, -opt, -plo) %>% 
    mutate(
      medest = map(depsub, function(x){
        
        # estimate centroids, minq, maxq
        out <- x %>% 
          select(val, abundances) %>% 
          gather('var', 'val', everything()) %>% 
          group_by(var) %>% 
          summarise(
            lov = quantile(val, 0.05, na.rm = T), 
            mdv = quantile(val, 0.5, na.rm = T),
            hiv = quantile(val, 0.95, na.rm = T)
          ) %>% 
          gather('est', 'val', lov, mdv, hiv)
...
renderPlot({
  ...
  # abu centroid segs
  abuseg <- cendat %>% 
    filter(!(var %in% 'val' & est %in% c('hiv', 'lov'))) %>% 
    filter(!(var %in% 'abundances' & est == 'mdv')) %>% 
    select(-var) %>% 
    spread(est, val)
  
  # val centroid segs
  valseg <- cendat %>% 
    filter(!(var %in% 'abundances' & est %in% c('hiv', 'lov'))) %>% 
    mutate(
      est = ifelse(var == 'abundances' & est == 'mdv', 'yv', est),
      est = ifelse(var == 'val' & est == 'mdv', 'xv', est)
    ) %>% 
    select(-var) %>% 
    spread(est, val)
  ...
  # plot
  p <- ggplot() +
    geom_point(data = obsdat, aes(x = val, y = abundances)) +
    geom_errorbar(data = abuseg, aes(x = mdv, ymin = lov, ymax = hiv, group = dep), size = 1, alpha = 0.7) +
    geom_errorbarh(data = valseg, aes(x = xv, y = yv, xmin = lov, xmax = hiv, group = dep), size = 1, alpha = 0.7) + 
    geom_point(data = meddat, aes(x = val, y = abundances), size = 4, pch = 21, fill = 'grey') +
    scale_x_continuous(vr) +
    scale_y_continuous("log-abundance") +
    facet_wrap(~dep, ncol = 2) +
...
renderUI({
  ...
  cendat <- medests %>% 
    select(-depsub) %>% 
    unnest %>% 
    mutate(var = ifelse(var %in% 'val', vr, var)) %>% 
    select(-vr) %>% 
    spread(est, val) %>% 
    select(dep, var, lov, mdv, hiv)
  
  HTML(knitr::kable(cendat, caption = 'Estimated 5th, median, and 95th percentiles for selected variables with abundance.', format = 'html') %>% 
    kable_styling(full_width = T, font_size = 14))
