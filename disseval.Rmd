...
```{r}
cffsmlt <- glmulti(disscat ~ oxy + temp + arag, data = toeval, level = 1, method = 'h', crit = 'aicc', plotty = F, report = F, fitfunction = 'glm', family = 'binomial') %>%
      coef %>%
      data.frame %>%
      rownames_to_column('var')
  
  # global glm
  globs <- glm(disscat ~ oxy + temp + arag, data = toeval, family = 'binomial', na.action = na.pass)
...
toplo <- cffsmlt %>% 
  ...
  select(var, Estimate, Uncond..variance, Importance) %>% 
  gather('est', 'val', - var)
...
p1 <- ggplot(toplo, aes(x = var, y = val)) +
  geom_bar(stat = 'identity', position = 'dodge', colour = 'black') +
  facet_wrap(~ est, ncol = 3,scales = 'free_y') +
  thm +
...
```{r results = 'asis'}
HTML(knitr::kable(drdgsub, format = 'html') %>% 
