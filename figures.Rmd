...
datbio <- datbio %>% 
  ...
  select(CTD, external, avelen) %>% 
  gather('var', 'val', external, avelen) %>% 
  group_by(var) %>% 
...
ggord(ord, axes = c('1', '2'), vec_ext = 2, ptslab = T, parse = T, obslab = T, labcol = 'tomato1', repel = T, coord_fix = F, addsize = 3) + 
