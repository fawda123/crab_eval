...
datenv <- envdatdelt %>%
  filter(var %in% c('pCO2', 'Oxygen', 'Temperature', 'Chla', 'Calcitedelt', 'Temperaturedelt', 'Oxygendelt', 'Chladelt')) %>% 
  filter(depth %in% 60) %>% 
...
mod1 <- lm(Int.diss ~ (pCO2 + Temperature + Chla), data = tomod)
summary(mod1)
summary(step(mod1, direction = 'backward'))

mod2 <- lm(Ext.diss ~ (Calcitedelt + Oxygendelt + Temperaturedelt + Chla), data = tomod)
summary(mod2)
