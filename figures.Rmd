...
```{r, fig.height = 4, fig.width = 7, fig.cap = 'Present day pH and dissolved inorganic carbon at stations where crabs were sampled (including those sampled but no crabs were found) and the relationship between external and internal dissoluation at stations where crabs were found.' }

toplo1 <- envdatdelt %>% 
  filter(var %in% c('DIC', 'pH')) %>% 
  filter(depth %in% 60) %>% 
  filter(CTD %in% crbs$CTD) %>% 
  dplyr::select(CTD, var, val) %>% 
  spread(var, val)

p1 <- ggplot(toplo1, aes(y = pH, x = DIC)) + 
  geom_point() + 
  pbase + 
...
p2 <- ggplot(toplo2, aes(y = external, x = internal)) + 
  geom_point() + 
  xlab('Internal') + 
  ylab('External') + 
...
summary(lm(external ~ internal, toplo2))
```
